...
const listLatestMangaUpdate = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const latestMangaUpdates = sqlResult.rows.map((latestMangaUpdate) => LatestMangaUpdate.fromSQL(latestMangaUpdate)).map((latestMangaUpdate) => latestMangaUpdate.getObject())
